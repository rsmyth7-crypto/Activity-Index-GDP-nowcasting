...
# IMPORTANT: Please upload the file `NAQ03.20251204T171251.csv` to the current Colab environment.
# The cell below will then move it to the correct project folder.

gdp_filename = "NAQ03.20251204T171251.csv"  # This is based on current name of the GDP data file
...
from pytrends.request import TrendReq
import pandas as pd
from pytrends import exceptions # Import exceptions for TooManyRequestsError
...
try:
    # Modified timeframe to fetch data up to the end of the next year (2025-12-31)
    # This ensures we have recent high-frequency data for nowcasting.
    pytrends.build_payload(keywords, geo='IE', timeframe='2010-01-01 2026-01-01')
    df_trends = pytrends.interest_over_time()

    project_root = '/content/economic_search_project'  # Ensure project_root is defined or accessible
    output_path = os.path.join(project_root, 'data/raw', 'google_trends.csv')
    df_trends.to_csv(output_path)
    print(f"Google Trends data successfully saved to {output_path}")

except exceptions.TooManyRequestsError:
    print("Error: Google returned a 'Too Many Requests' (429) error.")
    print("This usually happens when too many requests are sent to Google Trends from this IP address in a short period.")
    print("Please wait for a few minutes and try rerunning this cell.")
    print("If the error persists, consider restarting the Colab runtime (Runtime -> Restart runtime) to get a new IP address, then rerun the cells.")
    # Set df_trends to None to prevent subsequent cells from failing if they rely on it
    df_trends = None
except Exception as e:

# -*- coding: utf-8 -*-
"""5.TrainoOutofSampleNowcast

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1bdnYMBn3dy7zZON89CgnyR_hpBNBFx9E

## Train Final Nowcasting Model (using Hospitality Index)


Train a Random Forest Regressor model on the entire available historical data, using `GDP_lag1` and `hospitality_index` as features, to prepare it for out-of-sample nowcasting.
"""
...
### Subtask:
Identify the latest available values for `GDP_lag1` and `hospitality_index` to construct the feature vector for the next quarter's GDP growth prediction (i.e., a date where actual GDP is not yet available).
...
Use the trained Random Forest model to predict the GDP growth for the next quarter, and then calculate the nowcasted GDP value based on the previous quarter's actual GDP and the predicted growth.
